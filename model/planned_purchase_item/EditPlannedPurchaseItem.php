...
class EditPlannedPurchaseItem implements ActionPlannedPurchaseItem
{
    public function execute(PlannedPurchaseItemDAO $plannedPurchaseItemDAO): void
    {
        ...
        //Query
        $status = $plannedPurchaseItemDAO->editPlannedPurchaseItem();
        if(!$status)
        {
            $_SESSION['msg-errors'] = ["Não foi possível editar o item da lista."]; 
            header('Location: ../view/pages/list-current-purchase.php');
            return;
        }

        //Response
        $_SESSION['msg-success'] = "Item da lista editado com sucesso!"; 
        header('Location: ../view/pages/list-current-purchase.php');
    }
}